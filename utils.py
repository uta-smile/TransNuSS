...
def test_single_volume(image, label, net, classes, patch_size=[256, 256], test_save_path=None, case=None, z_spacing=1):
    ...
    if len(image.shape) == 4:
        ...
        with torch.no_grad():
            ...
            # out = torch.argmax(torch.softmax(prediction[0], dim=1), dim=1).squeeze(0)
    ...
    if test_save_path is not None:
        ...
        # output_image = output_image.resize(output_size, Image.NEAREST)
        output_image.save('%s/%s' % (test_save_path, case.replace('_image.', '_mask.')))
